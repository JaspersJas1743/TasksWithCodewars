...
You can immediately see that going "NORTH" and immediately "SOUTH" is not reasonable, better stay to the same place! So the task is to give
to the man a simplified version of the plan.A better plan in this case is simply: ["WEST"] or { "WEST" } or [West]
Other examples:
In ["NORTH", "SOUTH", "EAST", "WEST"], the direction "NORTH" + "SOUTH" is going north and coming back right away.
The path becomes ["EAST", "WEST"], now "EAST" and "WEST" annihilate each other, therefore, the final result is [] (nil in Clojure).
In ["NORTH", "EAST", "WEST", "SOUTH", "WEST", "WEST"], "NORTH" and "SOUTH" are not directly opposite but they become directly opposite after the
...
{  
    ...
    private static bool CheckPairForOpposite(string one, string two) => _replace[one] == two;

    public static string[] DeleteOppositeDirections(string[] directions)
    {
        string[] result = new string[directions.Length]; Array.Copy(directions, result, directions.Length);

        for (int index = 0; index < directions.Length - 1; ++index)
            if (CheckPairForOpposite(directions[index], directions[index + 1]))
                directions = directions[0..index].Concat(directions[(index + 2)..^0]).ToArray();

        if (Enumerable.SequenceEqual(directions, result)) return directions;

        return DeleteOppositeDirections(directions);
    }
}
...
class Program
{
    public static void Main(string[] args)
    {
        ...
        foreach (var direction in DirReduction.DeleteOppositeDirections(new string[]{"NORTH", "WEST", "SOUTH", "EAST"}))
            Console.Write($"{direction} ");
        Console.WriteLine("\nТест 2");
        foreach (var direction in DirReduction.DeleteOppositeDirections(new string[]{"NORTH", "SOUTH", "SOUTH", "EAST", "WEST", "NORTH", "WEST"}))
            Console.Write($"{direction} ");
    }
}